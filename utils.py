...
def parse_m3u_playlist(content: str):
    streams = []
    content = content.replace("#EXTM3U", "").replace("#TOTAL-VS-MATCHES:", "").replace("#LAST-UPDATED:", "")
    ...
    for idx, block in enumerate(blocks):
        ...
        if logo_m: stream["logo"] = logo_m.group(1).strip()
            
        group_m = re.search(r'group-title="([^"]+)"', block, re.IGNORECASE)
        if group_m: stream["group"] = group_m.group(1).strip()
        ...
        if stream["url"]: streams.append(stream)
