...
async def create_short_link(stream_url, referer, origin, cookie, user_agent, source_url, title=""):
    ...
    doc = {
        "short_id": short_id, "stream_url": stream_url, "title": title, 
...
async def add_user(user_id): await users_col.update_one({"user_id": user_id}, {"$setOnInsert": {"user_id": user_id, "joined_at": datetime.utcnow(), "is_banned": False}}, upsert=True)
async def get_all_users(): return [doc["user_id"] async for doc in users_col.find({"is_banned": {"$ne": True}})]
async def is_user_banned(user_id): user = await users_col.find_one({"user_id": user_id}); return user.get("is_banned", False) if user else False
async def toggle_ban_user(user_id, status): await users_col.update_one({"user_id": user_id}, {"$set": {"is_banned": status}})
async def add_m3u_source(url, target="both"): await sources_col.update_one({"url": url}, {"$set": {"url": url, "target": target, "added_at": datetime.utcnow()}}, upsert=True)
async def remove_m3u_source(url): await sources_col.delete_one({"url": url}); await posted_col.delete_many({"source_url": url}); await links_col.delete_many({"source_url": url}); return 0,0
